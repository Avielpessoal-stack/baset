...
def regressao_linear_manual(x, y):
    """Implementa regressão linear sem scikit-learn"""
    n = len(x)
    if n < 2:
        return None, None, 0
...
def realizar_regressao(x, y, inicio_emergencia=0):
    ...
    # Usar regressão manual
    slope, intercept, r2 = regressao_linear_manual(x_reg_clean, y_reg_clean)
    
    if slope is None:
        return None, None, 0, None, None
    
    # Calcular predições para o gráfico
    y_pred = slope * x_reg_clean + intercept
    
    return slope, intercept, r2, x_reg_clean, y_pred
